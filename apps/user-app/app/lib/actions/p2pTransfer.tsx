...
import prisma from "@repo/db/client";
import {sendSMS} from "@repo/notification/src/sms";
...
export async function p2pTransfer(to: string, amount: number) {
  ...
  const receiverPhone = `+91${toUser.number}`;
...
await tx.$queryRaw`SELECT * FROM "Balance" WHERE "userId" = ${Number(from)} FOR UPDATE`; ///this locks the row for the duration of the transaction
 ...
 sendSMS(receiverPhone, `Your Account is credited with  ₹${amount}. Thank you `); //send sms to receiver
